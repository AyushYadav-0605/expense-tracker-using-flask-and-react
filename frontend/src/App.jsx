...
function App() {
  ...
  //  fallback if backend returns empty items but total > 0
  if (items.length === 0 && data.total > 0) {
    const r2 = await fetch(`${API}/expenses?per_page=100`);
  ...
  const loadMonthlyReport = useCallback(async () => {
    ...
    const data = await r.json();
    const fixed = (data.category_totals_for_chart || []).map((item) => ({
  name: item.name,
  value: item.amt,
}));
setMonthlyByCat(fixed);
  }, [year, month]);
...
setTrendData(series);
  ...
  const pageTotal = expenses.reduce((a, e) => a + Number(e.amount), 0);
